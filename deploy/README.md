# NexusAPI Deployment Files

This directory contains files for deploying NexusAPI on Linux servers and Apple-silicon Macs.
...
| `NexusAPI.service` | Systemd service unit file |
| `NexusAPI-datamanagementd.service` | datamanagementd systemd service unit file |
...
Apple-silicon Macs running macOS 26 can run the complete NexusAPI, PostgreSQL, and Redis stack with Apple `container` 1.1.0 or newer:
...
The script uses Apple named volumes, starts dependencies in order, and performs live readiness checks. The application container supervises the NexusAPI process so the Web UI's update-and-restart flow can relaunch an updated binary. It does not provide host-level automatic startup; run `./apple-container.sh up` after a host reboot. Docker Compose remains the recommended production deployment path.
...
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/NexusAPI/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs NexusAPI | grep "admin password"
   ```

### Startup and Database Recovery

NexusAPI applies database migrations during application startup. PostgreSQL can
...
`NexusAPI.service`; the application retry covers transient database startup,
while systemd remains the supervisor for permanent process exits. For
Kubernetes, use a PostgreSQL readiness probe and retain the NexusAPI startup
...
# Restart NexusAPI only
docker compose -f docker-compose.local.yml restart NexusAPI
...
# Restart NexusAPI only
docker compose restart NexusAPI
...
| `ADMIN_EMAIL` | No | `admin@NexusAPI.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf NexusAPI-complete.tar.gz deployment/

# Transfer to new server
scp NexusAPI-complete.tar.gz user@new-server:/path/to/destination/
...
NexusAPI supports three methods to connect to Gemini:
...
1. Download the latest release from [GitHub Releases](https://github.com/Wei-Shaw/NexusAPI/releases)
2. Extract and copy the binary to `/opt/NexusAPI/`
3. Copy `NexusAPI.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/NexusAPI.service
   ```
...
The main config file is at `/etc/NexusAPI/config.yaml` (created by Setup Wizard).
...
```
/opt/NexusAPI/
├── NexusAPI              # Main binary
├── NexusAPI.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 NexusAPI
...
# View detailed logs
docker compose logs --tail=100 NexusAPI
...
# View recent logs
sudo journalctl -u NexusAPI -n 50

# Check config file
sudo cat /etc/NexusAPI/config.yaml
...
NexusAPI supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.NexusAPI.org](https://tls.NexusAPI.org/)** to get TLS fingerprint information for different devices and browsers.
